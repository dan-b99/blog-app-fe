...
@Component({
  ...
  <div class="container mt-5">
    <mat-card class="p-5">
      <div class="row">
        <div class="col-auto">
          <h2 class="display-2">Benvenuto! Registrati:</h2>
        </div>
      </div>
  ...
  sendRegistration() {
    ...
    this.userService.register(userToRegister).subscribe({
      next: (registered: UtenteOutput) => {
        this.snackBar.open("Operazione riuscita");
        this.router.navigateByUrl("/login");
      },
      error: (respErr: HttpErrorResponse) => this.snackBar.open(respErr.error.message)
    });
  }
}
